// For circular displays of posts
...
function SimpleDiskParticle(posX, posY, text, title, name, address, len, rad) {
		this.x = posX;
		this.y = posY;
		this.velX = 0;
		this.velY = 0;
...
SimpleDiskParticle.prototype.drawToContext = function(theContext) {
	...
	theContext.arc(this.x, this.y, this.rad, 0, 2*Math.PI, false);
	theContext.closePath();
	theContext.fill();
	theContext.font = 18 + 6*(1 - this.len/500) + "px serif";
	theContext.fillStyle="white";
	// displays wrapped text of post 
	textWrapper(theContext, this.text, this.x - 0.85*this.rad, this.y - 0.55*this.rad + 0.5*this.rad*(1 - this.len/500), this.len, this.rad); 
